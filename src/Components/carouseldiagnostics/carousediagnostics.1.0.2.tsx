...
const MiniCarousel: React.FC<MiniCarouselProps> = ({
  columns,
  onGestureComplete,
  isActive,
  hasMotionPermission,
    ...
    const updateDimensions = () => {
      if (containerRef.current) {
        ...
        const availableWidth =
          containerWidth - 2 * horizontalPadding - totalGapSpace;
  ...
  const handleDragStart = () => {
    ...
    deviceMotionData.current = [];
    deviceMotionStart.current =
      deviceMotionData.current.length > 0
  ...
  const handleDrag = (
    ...
    if (trajectoryData.current.length > 0) {
      const lastPoint =
        trajectoryData.current[trajectoryData.current.length - 1];
  ...
  const handleDragEnd = (
    ...
    const pressureAvg =
      touchPressures.current.length > 0
        ? touchPressures.current.reduce((a, b) => a + b, 0) /
          touchPressures.current.length
        : 0;
    const pressureMax =
      touchPressures.current.length > 0
    ...
    const motionMetrics = calculateDeviceMotion(
      deviceMotionData.current,
      deviceMotionStart.current
    );
    ...
    const peakAcceleration = calculatePeakAcceleration(
      velocityHistory.current,
      duration
    );
    ...
    const velocityCV =
      avgVelocity > 0 ? Math.sqrt(velocityVariance) / avgVelocity : 0;
    ...
    // Retry detection
    const timeSinceLast =
      lastGestureTime.current > 0 ? Date.now() - lastGestureTime.current : 9999;
...
function detectGlide(
  ...
  if (
    distance > GLIDE_DISTANCE_MEDIUM &&
    velocity > GLIDE_VELOCITY_MEDIUM &&
    peakAccel > GLIDE_ACCELERATION_MEDIUM
  )
    return true;
  ...
  if (
    distance > GLIDE_DISTANCE_ENERGETIC &&
    (velocity > GLIDE_VELOCITY_HIGH || peakAccel > GLIDE_ACCELERATION_HIGH)
  )
    return true;
...
export default function CarouselSwipeDiagnostic() {
  ...
  const [screen, setScreen] = useState<
    "setup" | "calibration" | "test" | "complete"
  >("setup");
  ...
  // Test state
  const [phase, setPhase] = useState<"flicks" | "interstitial" | "glides">(
    "flicks"
  );
  const [gestureData, setGestureData] = useState<GestureData[]>([]);
  const [flickCount, setFlickCount] = useState(0);
  const [glideCount, setGlideCount] = useState(0);
  const [activeCarousel, setActiveCarousel] = useState<"1-column" | "2-column">(
    "1-column"
  );
  ...
  useEffect(() => {
    ...
    const hasMotionSupport =
      typeof DeviceMotionEvent !== "undefined" && !!window.DeviceMotionEvent;
  ...
  const handleGestureComplete = (
    carousel: "1-column" | "2-column",
    partialData: Partial<GestureData>
  ) => {
    ...
    if (updatedGestures.length >= 4) {
      ...
      const flaggedGestures = updatedGestures.map((gesture) => {
        ...
        const isStraightnessOutlier = detectOutlier(
          gesture.straightness,
          straightnesses
        );
        ...
        return {
          ...gesture,
          isOutlier:
            isVelocityOutlier || isDistanceOutlier || isStraightnessOutlier,
          outlierReasons:
            [
  ...
  const downloadCSV = () => {
    ...
    const dateStr = `${String(timestamp.getDate()).padStart(2, "0")}${String(
      timestamp.getMonth() + 1
    ).padStart(2, "0")}${String(timestamp.getFullYear()).slice(-2)}_${String(
      timestamp.getHours()
    ).padStart(2, "0")}${String(timestamp.getMinutes()).padStart(2, "0")}`;
    const filename = `${
      setupData.name || "Anonymous"
    }_CarouselSwipeDiagnostic_${dateStr}.csv`;
              ...
              <div style={{ display: "flex", gap: "8px" }}>
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid #E5E5E5",
                    background:
                      setupData.carouselPosition === "top" ? "#000" : "#fff",
                    color:
                      setupData.carouselPosition === "top" ? "#fff" : "#000",
                    fontSize: "16px",
                    fontWeight: 500,
                    cursor: "pointer",
                    fontFamily,
                ...
                <button
                  ...
                  style={{
                    ...
                    border: "1px solid #E5E5E5",
                    background:
                      setupData.carouselPosition === "bottom" ? "#000" : "#fff",
                    color:
                      setupData.carouselPosition === "bottom" ? "#fff" : "#000",
                    fontSize: "16px",
                    fontWeight: 500,
                    cursor: "pointer",
                    fontFamily,
            ...
            >
              ...
              <button
                ...
                style={{
                  ...
                  border: "1px solid #E5E5E5",
                  background:
                    permissionsGranted.motion || permissionsGranted.orientation
                      ? "#4CAF50"
                      : "#fff",
                  color:
                    permissionsGranted.motion || permissionsGranted.orientation
        ...
        >
          ...
          {calibrationStep === "thumb" && !thumbCaptured && (
            ...
            >
              ...
              >
                Hover your thumb over the screen where it naturally lands, then
                press down anywhere.
              </p>
              <div
                style={{
          ...
          {calibrationStep === "thumb" && thumbCaptured && (
            <>
              ...
              >
                Thumb rest: ({calibrationData.thumbRestPosition.x.toFixed(0)},{" "}
                {calibrationData.thumbRestPosition.y.toFixed(0)})
              </p>
              <button
      ...
      >
        ...
        <div
          style={{
            flex: 1,
            padding: "16px",
            paddingBottom: showDownloadButton
              ? "200px"
              : "max(16px, env(safe-area-inset-bottom, 0px) + 16px)", // Add space for fixed download section and safe area
            display: "flex",
            flexDirection: "column",
            gap: "32px",
        ...
        >
          <MiniCarousel
            columns={1}
            onGestureComplete={(data) =>
              handleGestureComplete("1-column", data)
            }
          ...
          <MiniCarousel
            columns={2}
            onGestureComplete={(data) =>
              handleGestureComplete("2-column", data)
            }
            isActive={phase !== "interstitial"}
            hasMotionPermission={permissionsGranted.motion}
          />
