...
export default function AdaptiveCarousel({
  ...
  useEffect(() => {
    ...
    const widthForCardFaces = availableSpace - totalGaps - peakAmount
      ...
      >
        ...
        >
          {childrenArray.map((child, index) => {
            // Calculate item width (last item gets extra space for peek)
            let finalItemWidth = itemWidth
            if (index === totalItems - 1 && totalItems > 0 && peakAmount > 0) {
              const spaceToFill = peakAmount + gap
              finalItemWidth = itemWidth + spaceToFill - horizontalPadding
            }
